...
export default async function InfoCard() {
  ...
  return (
    <section className={cn(flexColCenter, `px-5 pb-32`)}>
      <header
        className={cn(`justify-between absolute top-0 left-0 bg-white w-full`)}
      >
          ...
          <div className={cn(flexColCenter)}>
            ...
            {session?.user?.name ? (
              <div className={cn(flexRowCenter, titleMd)}>
                <h1
                  className={cn(primary, `underline underline-offset-4 mr-2`)}
                >
                  {session?.user?.name}
                </h1>
                <h1>집사님</h1>
      ...
      <div>
        ...
        <div className={"flex flex-row my-6"}>
          ...
          <div>
            <p className={bodyLg}>
              반려동물의 세세한 정보까지 한눈에 확인하고,
            </p>
            <p className={cn(optionalText, `text-[13px]`)}>
              반려동물의 기본정보와 추가적으로 유의사항을 작성해 쉽게 확인하며
              대비할 수 있어요.
            </p>
          </div>
        </div>
